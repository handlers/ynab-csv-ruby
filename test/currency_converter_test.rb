...
class CurrencyConverterTest < Minitest::Test
  def test_to_cents_simple
    ...
    assert_equal CurrencyConverter.new(currency_string).to_cents, 1000
  end

  def test_to_cents_complex
    currency_string = "$10.99"

    assert_equal CurrencyConverter.new(currency_string).to_cents, 1099
  end

  def test_to_cents_less_than_a_dollar
    currency_string = "$.99"

    assert_equal CurrencyConverter.new(currency_string).to_cents, 99
  end
